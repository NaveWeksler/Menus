...
export const withDB =
	(next: nextFunction) => async (req: NextApiRequest, res: NextApiResponse) => {
		await connect();
		return next(req, res);
	};

const expectMethod =
	(next: nextFunction, type: string) =>
...
export const withContract =
	<Input, Output, Request extends NextApiRequest = NextApiRequest>(
		next: (
			req: Omit<Request, 'body'> & { body: z.infer<typeof bodyValidator> },
			res: NextApiResponse<Output>
		) => any,
		bodyValidator: z.Schema<Input>
	) =>
	(req: NextApiRequest | AuthRequest, res: NextApiResponse) => {
		if (req.method === 'post' && !bodyValidator.safeParse(req.body).success)
			return res.status(400).end();

		return next(
			req as Omit<Request, 'body'> & { body: z.infer<typeof bodyValidator> },
			res as NextApiResponse<Output>
		);
	};
